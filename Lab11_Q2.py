#This code uses Monte Carlo simulations to simulate protein folding mechanisms. 

#This code is the modified version of the code provided by Nicolas Grisuard
#Modified by Milica Ivetic, December 2022
...
n = int(9.5e6)  # number of Monte Carlo steps

"""CHANGING TEMPERATURE CODE FOR PART 2D and 2E"""
T_f = 0.5
T_steps = 19
T_i = 10
T_array = np.zeros(n)
...
for j in range(n):
    ...
    if monomer_array[new_monomer_pos[0], new_monomer_pos[1]] == 0:
        ...
        if distance_okay:
            ...
            if random() < np.exp(-(new_energy-energy)/T_better): #CHANGED CODE HERE
                # make switch
                energy = new_energy
...
for i in range(T_steps):
    mean_energies.append(np.mean(energy_array[i*500000:(i+1)*500000]))
    std_energies.append(np.std(energy_array[i*500000:(i+1)*500000]))
